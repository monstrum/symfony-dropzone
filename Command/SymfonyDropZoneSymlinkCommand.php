 ...
 */
namespace Monstrum\SymfonyDropZoneBundle\Command;
...
/**
 * Command to check and create enyo-dropzone symlink into MonstrumSymfonyDropZoneBundle
 *
 * @author phiamo <phiamo@googlemail.com>
 * @author MisatoTremor <tlot@blackblizzard.org>
 * @author Erwin Eu <eu.erwin@gmail.com>
 */
class SymfonyDropZoneSymlinkCommand extends ContainerAwareCommand
{
    public static $monstrumSymfonyDropZoneBundleName = "monstrum/symfony-dropzone-bundle";
    public static $dropzoneName = "enyo/dropzone";
    public static $sourceSuffix = 'dist';
    public static $targetSuffix = 'Resources/public';
    public static $pathName = 'SymfonyDropZone';
    ...
    protected function configure()
    {
        $this
            ...
            ->addArgument('pathTo' . static::$pathName, InputArgument::OPTIONAL, 'Where is enyo/dropzone located?')
            ->addArgument('pathToMonstrumSymfonyDropZoneBundle', InputArgument::OPTIONAL, 'Where is SymfonyDropZone located?')
            ->addOption('force', 'f', InputOption::VALUE_NONE, 'Force rewrite of existing symlink if possible!')
            ->addOption('manual', 'm', InputOption::VALUE_NONE, 'If set please specify pathTo' . static::$pathName . ', and pathToMonstrumSymfonyDropZone')
            ...
            ->setName('monstrum:symfony-dropzone:symlink')
            ->setHelp(<<<EOT
The <info>monstrum:symfony-dropzone:symlink</info> command helps you checking and symlinking/mirroring the enyo/dropzone library.
...
php app/console monstrum:symfony-dropzone:symlink <comment>--manual</comment> <pathToSymfonyDropZone> <pathToMonstrumSymfonyDropZone>

Defaults if installed by composer would be :

pathToSymfonyDropZone:    ../../../../../../../../vendor/enyo/dropzone/dist
pathToMonstrumSymfonyDropZoneBundle: vendor/monstrum/symfony-dropzone-bundle/Monstrum/SymfonyDropZoneBundle/Resources/public/symfony-dropzone
...
EOT
     ...
     */
    protected function getDropZoneName()
    {
        return self::$dropzoneName;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('manual')) {
			list($symlinkTarget, $symlinkName) = $this->getSymfonyDropZonePathsFromUser();
            ...
            list($symlinkTarget, $symlinkName) = $cmanager->getSymlinkFromComposer(
                self::$monstrumSymfonyDropZoneBundleName,
                $this->getDropZoneName(),
                $options
            );
    ...
    protected function getSymfonyDropZonePathsFromUser()
    {
        $symlinkTarget = $this->input->getArgument('pathTo' . static::$pathName);
        $symlinkName = $this->input->getArgument('pathToMonstrumSymfonyDropZoneBundle');

        if (empty($symlinkName)) {
            throw new \Exception("pathToMonstrumSymfonyDropZoneBundle not specified");
        }

        if (!is_dir(dirname($symlinkName))) {
            throw new \Exception("pathToMonstrumSymfonyDropZoneBundle: " . dirname($symlinkName) . " does not exist");
        }
